...
export const UserDetailModal: VFC<Props> = memo((props) => {
  ...
  useEffect(() => {
    setUsername(user?.username ?? "ないよ");
    setName(user?.name ?? "ないよ");
    setEmail(user?.email ?? "ないよ");
    setPhone(user?.phone ?? "ないよ");
  }, [user]);
        ...
        <ModalBody mx={4}>
          <Stack spacing={4}>
            <FormControl>
              ...
              <Input value={phone} onChange={onChangeForm} isReadOnly={!isAdmin} />
            </FormControl>
          </Stack>
        </ModalBody>
        {isAdmin && (
